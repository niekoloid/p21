/**
	...
	SXFE_structured_dimension_calloutC.cpp								作成日：2000.4.18
...
/**
	...
	２．インターフェース
	void	SXFE_structured_dimension_calloutC(SXFD_StrC *str,SXFD_AggrC *aggr)
	引数:	型				引数名			内容
...
**/
SXFE_structured_dimension_calloutC::SXFE_structured_dimension_calloutC(SXFD_StrC *str,SXFD_AggrC *aggr){
	...
	m_Entity = DEF_STRUCTURED_DIMENSION_CALLOUT;
...
**/
void	SXFE_structured_dimension_calloutC::WriteP21(SXFPart21FileC* IN_file) {
	if(m_Flg){
		...
		((SXFInstanceC*)m_Data.GetPrev(pos))->WriteAttr(IN_file);
		IN_file->WriteString(")\n");
		IN_file->WriteString("STRUCTURED_DIMENSION_CALLOUT()\n");
		IN_file->WriteString(");\n");
