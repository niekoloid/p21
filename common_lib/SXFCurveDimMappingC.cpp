/**
	...
	SXFCurveDimMappingC.cpp								作成日：2007.01.29
...
/**
	...
	２．インターフェース
		void	SXFCurveDimMappingC::SXFCurveDimMappingC()
...
**/
SXFCurveDimMappingC::SXFCurveDimMappingC()
{
	m_ID1 = -1;
	m_ID2 = -1;
	m_ArcMapping = NULL;
...
/**
	...
	２．インターフェース
	void SXFCurveDimMappingC(SXFTableManagerC* IN_TableManager)
...
**/
SXFCurveDimMappingC::SXFCurveDimMappingC(SXFTableManagerC* IN_TableManager)
{
	m_ID1 = -1;
	m_ID2 = -1;
...
/**
	...
	２．インターフェース
	BOOL MappingStruct(CurveDim_Struct IN_Struct)
	引数:	型					引数名				内容
			CurveDim_Struct	IN_Struct			構造体
...
**/
int SXFCurveDimMappingC::MappingStruct(CurveDim_Struct IN_Struct,int IN_AssemType)
{
	Arc_Struct Arc_Str;
	...
	m_inst[1] = new SXFE_curve_dimensionC(m_str2,m_aggr1);
	...
	//                                    => CURVE_DIMENSION()
		...
		//アセンブリ要素が参照するIDとして、CURVE_DIMENSIONのIDを登録
...
**/
BOOL SXFCurveDimMappingC::WriteAP202(SXFPart21FileC *IN_File)
{
	if (m_ArcMapping != NULL)
...
**/
void SXFCurveDimMappingC::GetID(int &OUT_ID1,int &OUT_ID2)
{
	OUT_ID1 = m_ID1;
	OUT_ID2 = m_ID2;
}
...
/**
	...
	１．機能概要
	弧長寸法Mapping時に矢印のDIRECTIONの計算を行う。
...
**/
int SXFCurveDimMappingC::CalcArrowDir(CurveDim_Struct IN_Struct, double &OUT_Arrow1_x, double &OUT_Arrow1_y, double &OUT_Arrow2_x, double &OUT_Arrow2_y)
{
	//長さと角度の許容誤差を取得
	double eps,angle_eps;
	gl_Man->GetInfo().Get_Tolerance(eps,angle_eps);
	//弧長寸法のみで利用する特別な許容誤差
	//  弧長寸法では、フィーチャで角度で保持する寸法円弧始角終角度と、
			...
			if ((
				...
				if ( (fabs(vector2_x - vector1_x)<eps_sp&&fabs(vector2_y - vector1_y)<eps_sp)||//矢印１と２が近い
					...
					//Y1 = -X
						...
						//矢印1内外コードが0から2以外のとき
						SXFErrorMsgC::RecordError(SXFARRCODE,DEF_CURVE_DIMENSION,3,"-",IN_Struct.arr1_code2,DEF_CURVE_DIMENSION);
						return SXFARRCODE;
					}
					...
					//Y2 = +X
						...
						//矢印2内外コードが0から2以外のとき
						SXFErrorMsgC::RecordError(SXFARRCODE,DEF_CURVE_DIMENSION,3,"-",IN_Struct.arr2_code2,DEF_CURVE_DIMENSION);
						return SXFARRCODE;
					}
				}
				///////////////////////////////////////////////////////////////////
				...
				///////////////////////////////////////////////////////////////////
					...
					SXFErrorMsgC::RecordError(SXFARRORERROR,DEF_CURVE_DIMENSION,6,"-",IN_Struct.arr1_x,IN_Struct.arr1_y,IN_Struct.arr2_x,IN_Struct.arr2_y,DEF_CURVE_DIMENSION);
...
/*
		...
		///////////////////////////////////////////////////////////////////
			...
			SXFErrorMsgC::RecordError(SXFSAMEANGLE,DEF_CURVE_DIMENSION,4,"-",IN_Struct.sun_angle0,IN_Struct.sun_angle1,DEF_CURVE_DIMENSION);
			return SXFSAMEANGLE;
		}
*/
	}
...
void SXFCurveDimMappingC::CalcAngle(double IN_x1,double IN_y1,double IN_x2,double IN_y2,double& OUT_angle)
{
	double LA;
	double Ax,Ay;
...
void SXFCurveDimMappingC::CalcAngleSub(double IN_angle1,double IN_angle2,double& OUT_angle)
{
	double Ax,Ay;
...
void SXFCurveDimMappingC::CalcAngleSub2(double IN_angle1,double IN_angle2,double& OUT_angle)
{
...
//------------------------------------------------------------------------------------------------
//	Method Name:
//		SXFCurveDimMappingC::AngleIsInside
//	Exposition:
...
//------------------------------------------------------------------------------------------------
BOOL	SXFCurveDimMappingC::AngleIsInside( const double& dbAngleStart, const double& dbAngleEnd, const double& dbAngleCheck, const double& dbEps ) const
{
